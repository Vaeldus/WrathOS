...
class WrathOSConfigurator(Adw.Application):
    ...
    def on_activate(self, app):
        ...
        self.win.set_default_size(580, 650)
    ...
    def build_main_view(self):
        ...
        hint = Gtk.Label(label="Choose what to install. You can re-run this anytime from the app menu.")
        ...
        skip_btn.connect("clicked", lambda b: self.win.close())
    ...
    def on_bundle_toggled(self, check, bundle, inner):
        desc_label = inner.get_first_child().get_next_sibling().get_last_child()
        if check.get_active():
            inner.remove_css_class("bundle-card")
            inner.add_css_class("bundle-card-selected")
            desc_label.remove_css_class("bundle-desc")
    ...
    def update_count_label(self):
        selected = sum(1 for b in BUNDLES if self.checks[b["id"]].get_active())
        if selected == 0:
            ...
            self.count_label.set_text(
                f"Installing {selected} bundle{'s' if selected != 1 else ''} · Est. {selected * 2}-{selected * 4} min"
            )

    def on_install_clicked(self, btn):
        selected = [b for b in BUNDLES if self.checks[b["id"]].get_active()]
    ...
    def build_progress_view(self, selected):
        ...
        thread = threading.Thread(target=self.run_installs, args=(selected,))
        thread.daemon = True
        thread.start()
        ...
        for i, bundle in enumerate(selected):
            ...
            try:
                ...
                if result.returncode == 0:
                    ...
                    self.log(f"✗ {bundle['name']} failed: {result.stderr[:200]}")
    ...
    def build_done_view(self):
        done_box = Gtk.Box(orientation=Gtk.Orientation.VERTICAL, spacing=16)
        ...
        done_label = Gtk.Label(label="✓ Setup Complete")
